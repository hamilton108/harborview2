...
var author$project$Critters$Types$CacheReset = function (a) {
	return {$: 'CacheReset', a: a};
};
...
var author$project$Critters$Commands$resetCache = function (purchaseType) {
	var url = author$project$Critters$Commands$mainUrl + ('/resetcache/' + elm$core$String$fromInt(purchaseType));
	return A2(
		elm$http$Http$send,
		author$project$Critters$Types$CacheReset,
	...
	function (critMsg, model) {
		switch (critMsg.$) {
			...
			case 'DlgNewCritterOk':
				var cmd = function () {
					var _n1 = model.selectedPurchase;
					if (_n1.$ === 'Nothing') {
						return elm$core$Platform$Cmd$none;
					} else {
						var p = _n1.a;
...
var author$project$Critters$Commands$newDenyRule = F4(
	function (_n0, ruleType, _n1, hasMemory) {
		var oid = _n0.a;
		var val = _n1.a;
		if (ruleType.$ === 'NoRuleType') {
			return elm$core$Platform$Cmd$none;
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'ResetCache':
				return _Utils_Tuple2(
					model,
					author$project$Critters$Commands$resetCache(model.currentPurchaseType));
			default:
				if (msg.a.$ === 'Ok') {
					var s = msg.a.a;
					...
					var s = msg.a.a;
					return _Utils_Tuple2(
						A4(author$project$Common$ModalDialog$errorAlert, 'Error', 'CacheReset Error: ', s, model),
						elm$core$Platform$Cmd$none);
				}
		}
	});
...
var author$project$Common$Html$labelInputItem = F5(
	function (_n0, inputType, inputValue, htmlClass, event) {
		var caption = _n0.a;
		return A2(
			elm$html$Html$div,
			_List_fromArray(
				...
				[
					...
					_List_fromArray(
						[
							...
							elm$html$Html$Attributes$value(initVal)
						]),
					_List_Nil)
				]));
	});
...
var author$project$Common$ModalDialog$modalDialog = F5(
	function (title, dialogState, ok, cancel, content) {
		...
		var _n0 = author$project$Common$ModalDialog$dialogStatePrm(dialogState);
		var opc = _n0.a;
		var ptre = _n0.b;
		return A2(
			elm$html$Html$div,
...
var author$project$Critters$Types$ResetCache = {$: 'ResetCache'};
...
var author$project$Critters$Views$view = function (model) {
	var title = function () {
		var _n3 = model.currentPurchaseType;
		switch (_n3) {
			case 4:
	...
	var clazz = 'form-group form-group--elm';
	var _n0 = model.ruleValue;
	var rv = _n0.a;
	var _n1 = model.currentCritId;
	var oid = _n1.a;
	var _n2 = model.currentAccId;
	var accId = _n2.a;
	return A2(
		elm$html$Html$div,
		_List_Nil,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2(author$project$Common$Buttons$button, 'Reset Cache', author$project$Critters$Types$ResetCache)
							])),
						elm$html$Html$text(title)
					])),
